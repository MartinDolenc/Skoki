...
def login_get():
    """Prikaži formo za registracijo."""
    return bottle.template("Kar ena stran.html")

@bottle.post("/Ne upaš/")
def Ne_upas_post():
    """Najdi tekmovalca."""
    ime = bottle.request.forms.ime
    # Ali tekmovalec obstaja?
    c = baza.cursor()
    c.execute("SELECT 1 FROM TEKMOVALEC WHERE IME=?", [ime])
    print(c.fetchone())
    if c.fetchone():
        print('hello')
        # Tekmovalec obstaja
        return bottle.template("Tekmovalec template.html")
    '''
    else:
